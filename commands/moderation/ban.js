...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName("target").setDescription("target to ban").setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    if (target != null) {
      if (interaction.member.id != interaction.guild.ownerId) {
        if (
          ...
          return interaction
            .editReply({
              content:
                "You are not able to ban someone with a higher role than you!",
            })
            .then((msg) => {
      ...
      if (target.id === interaction.guild.ownerId) {
        return interaction
          .editReply("You can not ban the server owner!")
          .then((msg) => {
            setTimeout(() => msg.delete(), ms("5 seconds"));
          });
      }
      ...
      interaction
        .editReply({
          content: `Banned ${target.user.tag} successfully!`,
        })
        .then((msg) => {
          setTimeout(() => msg.delete(), ms("5 seconds"));
        });
