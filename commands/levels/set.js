...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("set")
    .setDescription("set command for leveling system")
    ...
    .addIntegerOption((option) =>
      option
        .setName("amount")
        .setDescription("amount you want to set it to")
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    if (type == "xp") {
      await Levels.setXp(user.id, interaction.guild.id, value);
      ...
      interaction.editReply(`Set ${user.user.tag} xp to ${value}`);
    }
    if (type == "level") {
      await Levels.setLevel(user.id, interaction.guild.id, value);
      await mongo().then(async (mongoose) => {
        try {
          let rewards = await rewardsSchema.find({
            guildId: interaction.guildId,
      ...
      interaction.editReply(`Set ${user.user.tag} to level ${value}`);
    }
  },
};
