...
class Rank {
    ...
    async build(ops = { fontX: "Manrope", fontY: "Manrope" }) {
        ...
        // apply username
        !this.data.renderEmojis ? ctx.fillText(`${name}`, 257 + 18.5, 82) : await Util.renderEmoji(ctx, name, 257 + 18.5, 82);
        ...
        if (discrim) {
            ...
            ctx.fillText(`#${discrim.substr(0, 4)}`, ctx.measureText(name).width + 20 + 335, 82);
        }
        ...
        if (this.data.level.display && !isNaN(this.data.level.data)) {
            ...
            ctx.fillText(this.data.level.displayText, 560 - ctx.measureText(Util.toAbbrev(parseInt(this.data.level.data))).width, 164);

            ctx.font = `bold 32px ${ops.fontX}`;
            ctx.fillStyle = this.data.level.color;
            ctx.textAlign = "end";
            ctx.fillText(Util.toAbbrev(parseInt(this.data.level.data)), 620, 164);
        }
        ...
        if (this.data.rank.display && !isNaN(this.data.rank.data)) {
            ...
            ctx.fillText(this.data.rank.displayText, 560 - ctx.measureText(Util.toAbbrev(parseInt(this.data.level.data)) || "-").width - 7 - ctx.measureText(this.data.level.displayText).width - 7 - ctx.measureText(Util.toAbbrev(parseInt(this.data.rank.data)) || "-").width, 164);

            ctx.font = `bold 32px ${ops.fontX}`;
            ctx.fillStyle = this.data.rank.color;
            ctx.textAlign = "end";
            ctx.fillText(Util.toAbbrev(parseInt(this.data.rank.data)), 550 - ctx.measureText(Util.toAbbrev(parseInt(this.data.level.data)) || "-").width - 7 - ctx.measureText(this.data.level.displayText).width, 164);
        }
