...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        ...
        await mongo().then(async (mongoose) =>{
            try {
                ...
                let background = "https://www.ambientum.com/wp-content/uploads/2019/10/via-lactea-estrellas-696x385.jpg"
               let cardPref = await card.findOne({guildId: interaction.guildId, userId})
               let textColor = cardPref.textColor || "RED"
               let progressbarColor = cardPref.progressbarColor || "RED"
               if(cardPref.backgroundImage){
                    background = cardPref.backgroundImage
                }
               else if(cardPref.backgroundColor){
                    backgroundType = "COLOR"
                    background = cardPref.backgroundColor
                }
           
        const target = await Levels.fetch(userId, interaction.guild.id, true)
        if(!target) return interaction.followUp(`${user.tag} does not have any levels within the server.`)
    ...
    rankcard.build()
        .then(data => {
        let attachment = new Discord.MessageAttachment(data, "RankCard.png");
